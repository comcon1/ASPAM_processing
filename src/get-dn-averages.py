...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'hi:o:d:n:t')
    ...
    for i in range(ndays):
        print( '%2d | %s - %s  --  %d' % (i, time2str(dayint[i][0]), time2str(dayint[i][1]), resday[i][1] ) )
    print( '=============' )
    print( 'DAY-LIGHT INTERVALS:' )
    for i in range(ndays):
        print( '%2d | %s - %s  --  %d' % (i, time2str(lightint[i][0]), time2str(lightint[i][1]), resli[i][1] ) )
    print( '=============' )
    print( 'DAY-NIGHT INTERVALS:' )
    print( len(nightint), len(resni) )
    for i in range(min(len(nightint), len(resni)) ):
        print( '%2d | %s - %s  --  %d' % (i, time2str(nightint[i][0]), time2str(nightint[i][1]), resni[i][1] ) )
    ...
    for i in range(ndays):
        light_interval_start = max(lightint[i][0], rca.df.index[0])
        light_interval_stop = min(lightint[i][1], rca.df.index[-1])
        ...
        _i = rca.df.index.searchsorted(light_interval_start)
        _j = rca.df.index.searchsorted(light_interval_stop)
        _dd = data[_i:_j,1]

        __daydistance = sum(_dd) * turnstometers
        ...
        if i >= len(nightint):
            ...
            __nightdistance = sum(_dd) * turnstometers
